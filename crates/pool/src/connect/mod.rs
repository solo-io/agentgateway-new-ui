...
impl Connected {
	...
	/// Set whether the connected transport is to an HTTP agentgateway.
	///
	/// This setting will affect if HTTP/1 requests written on the transport
	/// will have the request-target in absolute-form or origin-form:
	///
	/// - When `agentgateway(false)`:
	///
	/// ```http
	/// GET /guide HTTP/1.1
	/// ```
	///
	/// - When `agentgateway(true)`:
	///
	/// ```http
	...
	/// Determines if the connected transport is to an HTTP agentgateway.
...
pub(super) mod sealed {
	...
	pub trait ConnectSvc {
		...
		fn connect(self, internal_only: Internal, dst: http::Extensions) -> Self::Future;
	}

	impl<S, T> Connect for S
	where
		S: tower_service::Service<http::Extensions, Response = T> + Send + 'static,
		...
		T: Read + Write + Connection + Unpin + Send + 'static,
	{
		...
		fn connect(
			...
			_: Internal,
			dst: http::Extensions,
		) -> crate::service::Oneshot<S, http::Extensions> {
		...
		T: Read + Write + Connection + Unpin + Send + 'static,
	{
		type Connection = T;
		type Error = S::Error;
		type Future = crate::service::Oneshot<S, http::Extensions>;

		fn connect(self, _: Internal, dst: http::Extensions) -> Self::Future {
			crate::service::Oneshot::new(self, dst)
		}
	}
	...
	where
		S: tower_service::Service<http::Extensions, Response = T> + Send,
		S::Error: Into<Box<dyn StdError + Send + Sync>>,
		S::Future: Unpin + Send,
		T: Read + Write + Connection + Unpin + Send + 'static,
	{
